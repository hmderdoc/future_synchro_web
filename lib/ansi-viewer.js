...
ansi_viewer.list_slideshow_files = function (galleries, gallery, relative_dir, scope) {
    var cache_key;
    var pool = [];

    function push_gallery(target_gallery) {
        ansi_viewer.list_directories(target_gallery).forEach(function (dir_entry) {
            ansi_viewer.list_files(
                ...
                pool.push({
                    ...
                    gallery_name: target_gallery.name,
                    gallery_description: target_gallery.description,
                    relative_dir: dir_entry,
                    current_dir_label: dir_entry.length ? dir_entry : target_gallery.description,
                    name: file.name,
                    label: file.label
                });
            });
    ...
    cache_key = scope === 'all'
        ? 'all'
        : [scope, gallery && gallery.id ? gallery.id : '', relative_dir].join('|');

    if (Object.prototype.hasOwnProperty.call(ansi_viewer._slideshow_pool_cache, cache_key)) {
        return ansi_viewer._slideshow_pool_cache[cache_key];
    }

    if (scope === 'all') {
        galleries.forEach(function (entry) {
            push_gallery(ansi_viewer.get_gallery(entry.id));
        });
    ...
    pool.sort(function (a, b) {
        var ag = String(a.gallery_description || a.gallery_name || '').toLowerCase();
        var bg = String(b.gallery_description || b.gallery_name || '').toLowerCase();
        var ad = String(a.relative_dir || '').toLowerCase();
        var bd = String(b.relative_dir || '').toLowerCase();
        var af = String(a.name || '').toLowerCase();
        var bf = String(b.name || '').toLowerCase();
    ...
    ansi_viewer._slideshow_pool_cache[cache_key] = pool;
    return pool;
};

ansi_viewer.find_slideshow_index = function (pool, gallery_id, relative_dir, file_name) {
    var index = -1;
    var normalized_dir = ansi_viewer.normalize_relative_path(relative_dir);

    pool.forEach(function (entry, idx) {
        if (
            entry.gallery_id === gallery_id &&
            ansi_viewer.normalize_relative_path(entry.relative_dir) === normalized_dir &&
...
ansi_viewer.select_slideshow_file = function (pool, action, current_ref, order_mode) {
    var index;

    if (!pool.length) return null;

    index = ansi_viewer.find_slideshow_index(
        pool,
        current_ref && current_ref.gallery_id,
        current_ref && current_ref.relative_dir,
        current_ref && current_ref.file_name
    );
...
ansi_viewer.build_state = function (options) {
    ...
    if (slideshow_action.length) {
        slideshow_pool = ansi_viewer.list_slideshow_files(
            galleries,
            gallery,
            current_dir,
            slideshow_scope
    ...
    if (slideshow_index < 0 && current_file_name.length) {
        slideshow_index = ansi_viewer.find_slideshow_index(
            slideshow_pool,
            gallery.id,
            current_dir,
            current_file_name
