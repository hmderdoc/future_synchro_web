function get_splash() {
    var DefaultSplash = '';
    if (settings && settings.ftelnet_splash) {
        var f = new File(settings.ftelnet_splash);
        f.open('rb');
        var splash = base64_encode(f.read());
        f.close();
        return splash || DefaultSplash;
    } else {
        return DefaultSplash;
    }
}

function get_variant() {
    if (!settings) return 'norip.noxfer';
    return (settings.ftelnet_rip ? 'rip' : 'norip') + '.' + (settings.ftelnet_xfer ? 'xfer' : 'noxfer');
}

/* Returns the URL of the fTelnet bundle to load.
 *
 * This used to scrape embed-v2.ftelnet.ca once a day and inject whatever
 * <script src> came back. That made an unpinned third-party CDN a hard
 * dependency of the terminal, and any upstream rebuild could silently break
 * the truecolor / tap / speed patches, which reach into fTelnet internals.
 *
 * We now serve our own build from hmderdoc/fTelnet_enhanced. Install it with
 * deploy.sh in that repo; it drops the bundle in root/js/.
 *
 * The ?v= is the bundle's mtime so a redeploy busts both the browser cache
 * and any stale service worker entry - a cached bundle paired with a newer
 * terminal-iframe.html is exactly the mismatch sw.ssjs warns about.
 *
 * Keep the bundle in root/ftelnet/ beside its fonts/ dir. fTelnet resolves
 * its bitmap font PNGs relative to its own <script src>, so the two move
 * together or the terminal renders no glyphs.
 */
function get_url() {
    const rel = 'ftelnet/ftelnet.' + get_variant() + '.min.js';
    const f = new File(backslash(settings.web_root) + rel);
    if (!f.exists) {
        log(LOG_ERR, 'fTelnet bundle missing: ' + f.name +
            ' - run deploy.sh in the fTelnet_enhanced repo');
        return '';
    }
    return rel + '?v=' + f.date;
}
